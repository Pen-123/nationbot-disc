    ...
    async def full_reset(self, ctx):
        ...
        try:
            ...
            if not self.db.full_reset():
                await ctx.send("❌ Full reset failed. Check the bot logs for the database error.")
                return

            # CivilizationManager only has _civ_cache. The old code referenced
            # a nonexistent _cache_timestamps attribute, which crashed AFTER
            # Firestore had already been wiped.
