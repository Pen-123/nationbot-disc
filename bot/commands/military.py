...
class MilitaryCommands(commands.Cog):
    ...
    def _get_border_info(self, user_id: str) -> Dict[str, Any]:
        """Fetch border info from Firestore."""
    ...
    def _update_border(self, user_id: str, updates: Dict[str, Any]) -> bool:
        """Update border info in Firestore."""
        try:
    ...
    def _check_war(self, attacker_id: str, defender_id: str) -> bool:
        """Check if there is an ongoing war between two players."""
        ...
        try:
            ...
            if not unit_type:
                ...
                embed.add_field(name="Available Units",
                                value="`soldiers` - Basic infantry (50 gold, 10 food each)\n`spies` - Intelligence operatives (100 gold, 5 food each)",
                                inline=False)
    ...
    async def make_peace(self, ctx, target: Optional[guilded.Member] = None):
        try:
            ...
            # Check existing pending peace offer
    ...
    async def manage_cards(self, ctx, action: Optional[Literal["view", "use"]] = None,
                           card_name: str = None, target: Optional[guilded.Member] = None):
        try:
            user_id = str(ctx.author.id)
            if not await self.check_civil_war_and_proceed(ctx, user_id):
                return
            ...
            if action is None or action.lower() == 'view':
                # We need to fetch unlocked cards from Firestore (stored under civilization/cards subcollection)
                # For simplicity, we'll use the existing method to get card selection? Actually, unlocked_cards is a separate collection in old SQLite.
                # We'll adapt: we store unlocked cards in a subcollection "unlocked_cards" under civilization.
                # But to keep it simple, we'll just show a placeholder for now.
                # We'll implement a simple method to fetch cards (to be added in database.py if needed)
                # For now, we'll just say "Use the card system soon"
                await ctx.send("🎴 Card system is being migrated to Firestore. Please use `.cards view` later.")
                return
                ...
                if not card_name:
                    ...
                    return
                # Placeholder
                await ctx.send(f"❌ Card '{card_name}' usage is not yet implemented in Firestore migration.")
            else:
    ...
    async def upgrade_tech(self, ctx, branch: Optional[Literal["ground", "naval", "air"]] = None, amount: int = 1):
        ...
        try:
            ...
            # Optional: unlock cards on tech level milestones
            await ctx.send(embed=embed)
    ...
    async def train_boost(self, ctx, amount: int = 1):
        ...
        try:
            ...
            # Optionally update boosted_soldiers to min(soldiers, 300) – we'll handle that in the strength calculation.
            embed = create_embed(
                "⚔️ Training Level Up!",
...
class MilitaryCommands(commands.Cog):
    ...
    async def manage_cards(self, ctx, action: Optional[Literal["view", "use"]] = None,
        ...
        try:
            ...
            if action is None or action.lower() == 'view':
                # Placeholder – we'll need to implement card storage in Firestore
    ...
    async def upgrade_tech(self, ctx, branch: Optional[Literal["ground", "naval", "air"]] = None, amount: int = 1):
        try:
            if not branch:
                embed = create_embed("🔬 Military Tech Upgrade", "Upgrade your military technology for 500 gold per level.", guilded.Color.blue())
                embed.add_field(name="Branches", value="`ground` – improves soldiers and ground combat\n`naval` – improves ship strength\n`air` – improves plane strength", inline=False)
                embed.add_field(name="Usage", value="`.tech <branch> [amount]` (e.g., `.tech ground 2`)", inline=False)
                embed.add_field(name="Current Tech", value=f"Check with `.tech status`", inline=False)
                await ctx.send(embed=embed)
