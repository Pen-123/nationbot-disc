...
class DiplomacyCommands(commands.Cog):
    ...
    async def _alliance_id_autocomplete(self, interaction: guilded.Interaction, current: str) -> List[app_commands.Choice[str]]:
        uid = str(interaction.user.id)
        candidates = []
        for alliance_id, proposal in self.pending_alliances.items():
            if proposal["target_id"] == uid and datetime.now() < proposal["expires"]:
                label = f"{alliance_id} - {proposal['alliance_name']}"
                if current.lower() in label.lower():
                    candidates.append(app_commands.Choice(name=label[:100], value=alliance_id))
        return candidates[:25]

    async def _trade_id_autocomplete(self, interaction: guilded.Interaction, current: str) -> List[app_commands.Choice[str]]:
        uid = str(interaction.user.id)
        candidates = []
        for trade_id, trade in self.pending_trades.items():
            if trade["target_id"] == uid and datetime.now() < trade["expires"]:
                label = f"{trade_id} - {trade['offer_amount']} {trade['offer_resource']} for {trade['request_amount']} {trade['request_resource']}"
                if current.lower() in label.lower():
                    candidates.append(app_commands.Choice(name=label[:100], value=trade_id))
        return candidates[:25]
    ...
    async def accept_alliance(self, ctx, alliance_id: str):
        ...
        try:
            ...
            if not alliance:
                await ctx.send("❌ Alliance creation succeeded but not found. Contact admin.")
                return
    ...
    @app_commands.describe(
        target="Recipient civilization leader",
        resource_type="Resource to transfer",
